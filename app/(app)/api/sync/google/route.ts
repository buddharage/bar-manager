...
const TARGET_FOLDERS = ["Design", "Finances", "HR", "Operations", "Legal"];
...
async function backfillMissingEmbeddings(
	...
	const chunkedIds = new Set(
		(chunked || []).map((c: { document_id: number }) => c.document_id),
	);
	const needsEmbedding = docsWithoutChunks.filter(
		(d: { id: number }) => !chunkedIds.has(d.id),
	);
	...
	await processInBatches(
		...
		async (doc) => {
			try {
				...
				console.error(
					`Backfill embedding failed for doc ${doc.id}:`,
					err,
				);
			}
		},
...
async function incrementalSync(
	...
	const { upserted, embedded, skipped } = await syncFiles(
		changedFiles,
		existingDocs,
		supabase,
	);
	...
	for (const folderName of TARGET_FOLDERS) {
		updatedFolderIds[folderName] = [
			...new Set([
				...
				...[...knownFolderIds].filter(
					(id) => folderIdToName.get(id) === folderName,
				),
			]),
		];
	}
...
async function fullScan(
	...
	for (const folderName of TARGET_FOLDERS) {
		...
		if (!folderId) {
			console.warn(
				`Google Drive folder "${folderName}" not found, skipping`,
			);
			continue;
		}

		const discoveredIds = new Set<string>();
		const files = await listFolderFilesRecursive(
			folderId,
			folderName,
			discoveredIds,
		);
	...
	let deleted = 0;
	const staleIds = [...existingDocs.keys()].filter(
		(id) => !seenExternalIds.has(id),
	);
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (cursor) {
			const incremental = await incrementalSync(
				cursor,
				existingDocs,
				supabase,
			);
			if (incremental) {
				...
				console.log(
					"Drive Changes token expired, falling back to full scan",
				);
